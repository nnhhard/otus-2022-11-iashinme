...
import ru.iashinme.homework07.exception.ValidateException;
import ru.iashinme.homework07.mapper.GenreMapper;
import ru.iashinme.homework07.model.Genre;
import ru.iashinme.homework07.repository.GenreRepository;
...
public class GenreServiceImplTest {
    ...
    void shouldCorrectlyExceptionByDeleteGenre() {
        when(genreRepository.findById(any(Long.class))).thenReturn(Optional.empty());

        assertThatThrownBy(() -> genreService.deleteGenreById(-1))
                .isInstanceOf(ValidateException.class)
                .hasMessageContaining("Genre not find with id = " + -1);
    }
