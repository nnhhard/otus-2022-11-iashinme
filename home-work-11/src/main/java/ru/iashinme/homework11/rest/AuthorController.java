...
public class AuthorController {
    ...
    public Mono<Void> deleteById(@PathVariable("id") String id) {
        return bookRepository.existsBookByAuthor_Id(id)
                .flatMap(result -> {
                    if (result) {
                        return Mono.error(new RuntimeException("Exists books with this author"));
                    } else {
                        return Mono.empty();
                    }
                })
                .switchIfEmpty(authorRepository.deleteById(id))
                .cast(Void.class);
    }
    ...
    public Mono<AuthorDto> updateAuthor(@RequestBody AuthorDto authorDto) {
        return authorRepository.existsById(authorDto.getId())
                .flatMap(res -> {
                    if(res) {
                        return Mono.just(authorDto.toEntity());
                    } else {
                        return Mono.error(new RuntimeException("Author not found with id = " + authorDto.getId()));
                    }
                })
